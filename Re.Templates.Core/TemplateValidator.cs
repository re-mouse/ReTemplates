...
public class TemplateValidator
{
    ...
    {
        public Stack<string> OpenedArrayTagNames { get; } = new();
        public Stack<string> OpenedConditionTagNames { get; } = new();
        public Stack<string> OpenedTagNames { get; } = new();
    }

    public void Validate(TemplateFolder templateFolder)
    {
        ValidateFolder(templateFolder);
    }
    
    public void Validate(TemplateItem templateItem)
    {
        ValidateItem(templateItem);
    }
    
    private void ValidateFolder(TemplateFolder templateFolder)
    {
        Validate(templateFolder.Name);
        
        foreach (var directory in templateFolder.Folders)
        {
            ValidateFolder(directory);
        }
    ...
    private void ValidateItem(TemplateItem item)
    {
        ...
        {
            throw new FormatException($"Error occured while validated: {item.Name}. {exception.Message}");
        }
    }
